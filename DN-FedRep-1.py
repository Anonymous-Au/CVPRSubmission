...
from utils.aggregation import communication, communication2, communicationfedrep
...
def main(Location, dataset, method):
    ...
    parser.add_argument('--method', type=str, default='fedrep')
    ...
    server_model.initdgatal(test_loaders[5])
    ...
    mlp = [MLP(hidden_size=512, input_size=512, num_classes=31) for idx in range(client_num)]
    print(f"The network has {num_params} parameters.")
    server_model_pre = 1
    previous_nets = [1 for idx in range(0, client_num)]
    ...
    adv = [1 for idx in range(0, client_num)]
    ...
    for a_iter in range(args.iters):  # All Epoch
        ...
        with torch.no_grad():
            ...
            server_model, models = communicationfedrep(
...
if __name__ == '__main__':
    ...
    Location = './SavedModel/Food101/FedRep/lr=5e-5/practical/'
    method = '/FedRep/lr=5e-5/practical/'
    dataset = '/Food101/'
