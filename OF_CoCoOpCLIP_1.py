...
from utils.aggregation import communication, communication2, communicationpromptFL, communicationCocoOp
...
from nets.CoCoOpCLIP import TextEncoderCocoOp, PromptLearnerCocoOp
import time
...
def main(Location, dataset, method):
    ...
    parser.add_argument('--method', type=str, default='CocoOpCLIP')
    ...
    server_model_pre = 1
    adv = []
    previous_nets = [1 for idx in range(client_num)]
    prompt_learners = [PromptLearnerCocoOp(n_ctx_num=16, classnames=models[idx].labels, clip_model=server_model.model).to('cpu') for idx in range(client_num)]
    ...
    for a_iter in range(args.iters):  # All Epoch
        ...
        for wi in range(args.wk_iters):  # Each client local training epoch
            ...
            for client_idx, model in enumerate(models):
                if client_idx in args.test_envs:
                    ...
                    train(
                        args, model, train_test_loaders[client_idx], optimizers[client_idx], device,adv,test_train[0],
        ...
        with torch.no_grad():
            ...
            prompt_learners = communicationCocoOp(
...
if __name__ == '__main__':
    ...
    Location = './SavedModel/OfficeHome/CPRA/CocoOpCLIP/RN50/lr=5e-4/'
    method = '/CocoOpCLIP/RN50/lr=5e-4/'
