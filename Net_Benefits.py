...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', type=str, default='fedclip')
    ...
    optimizers = [ optim.AdamW(
        params=[ {'params': models[ idx ].fea_attn.parameters(),'betas':(args.beta1, args.beta2)},
                 {'params':mlp[idx].parameters()}], lr=args.lr, eps=args.eps, weight_decay=args.weight_decay)
    ...
    if not os.path.exists('./SavedModel/Ours'):
        os.makedirs('./SavedModel/Ours')
            ...
            for client_idx in range(client_num):
                if client_idx in args.test_envs:
                    ...
                    if (Val_Acc[0]+Val_Acc[1]+Val_Acc[2]) / 3 > Val_mean:
                        ...
                        torch.save(models[0].fea_attn.state_dict(), './SavedModel/Client1.pth')
                        torch.save(mlp[0].state_dict(), './SavedModel/mlp1.pth')
                        torch.save(models[1].fea_attn.state_dict(), './SavedModel/Client2.pth')
                        torch.save(mlp[1].state_dict(), './SavedModel/mlp2.pth')
                        torch.save(models[2].fea_attn.state_dict(), './SavedModel/Client3.pth')
                        torch.save(mlp[2].state_dict(), './SavedModel/mlp3.pth')
                    print(' Test site-{:d}| Validation Acc: {:.4f} | Bacc: {:.4f}'.format(client_idx, test_acc, bacc))

    # Testing
    TestingMetrics = [[],[],[],[]]
    with torch.no_grad():
        models[0].fea_attn.load_state_dict(torch.load('./SavedModel/Client1.pth'))
        models[1].fea_attn.load_state_dict(torch.load('./SavedModel/Client2.pth'))
        models[2].fea_attn.load_state_dict(torch.load('./SavedModel/Client3.pth'))
        server_model, models = communication(
            args, server_model, models, client_weights)
    for client_idx in range(client_num):
        if client_idx in args.test_envs:
            ...
            if client_idx == 0:
                mlp[client_idx].load_state_dict(torch.load('./SavedModel/mlp1.pth'))
                test_acc, bacc, f1, precision, recall, benefits, tp, fp, all, none = test(args, models[ client_idx ],
                                                                       test_loaders[ client_idx ], device,
                                                                       mlp[ client_idx ])
                ...
                mlp[ client_idx ].load_state_dict(torch.load('./SavedModel/mlp2.pth'))
                test_acc, bacc, f1, precision, recall, benefits, tp, fp, all, none = test(args, models[ client_idx ],
                                                                       test_loaders[ client_idx ], device,
                                                                       mlp[ client_idx ])
                net_benefits2 = benefits
                ...
                mlp[ client_idx ].load_state_dict(torch.load('./SavedModel/mlp3.pth'))
                test_acc, bacc, f1, precision, recall, benefits, tp, fp, all, none = test(args, models[ client_idx ],
                                                                       test_loaders[ client_idx ], device,
                                                                       mlp[ client_idx ])
                net_benefits3 = benefits
