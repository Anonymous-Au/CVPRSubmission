...
def main(Location, dataset, method):
    ...
    parser.add_argument('--dataset', type=str, default='ModernOffice31')
    ...
    parser.add_argument('--method', type=str, default='facmic')
    ...
    mlp = [MLP(hidden_size=512, input_size=512, num_classes=65) for idx in range(client_num)]
    print(f"The network has {num_params} parameters.")
    server_model_pre = server_model
    previous_nets = models
    adv = [LMMDLoss(num_classes=len(models[idx].labels)) for idx in range(client_num)]
...
if __name__ == '__main__':
    ...
    Location = './SavedModel/DomainNet/PQRSCI/FACMIC/lr=5e-5/'
    method = '/FACMIC/lr=5e-5/'
    dataset = '/DomainNet/PQRSCI/'
